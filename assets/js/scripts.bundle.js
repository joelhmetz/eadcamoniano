...
var KTMobilePanel = function() {
    var demoPanel = KTUtil.getByID('kt_mobile_panel');
    var offcanvas;
    ...
    var init = function() {
        offcanvas = new KTOffcanvas(demoPanel, {
            overlay: true,  
            baseClass: 'kt-demo-panel',
            closeBy: 'kt_mobile_panel_close',
            toggleBy: 'kt_mobile_panel_toggle'
        }); 
        ...
        if (typeof offcanvas !== 'undefined' && !$.isEmptyObject(offcanvas)) {
            offcanvas.on('hide', function() {
                alert(1);
                var expires = new Date(new Date().getTime() + 60 * 60 * 1000); // expire in 60 minutes from now
                Cookies.set('kt_mobile_panel_shown', 1, { expires: expires });
            });
        }
    }
    ...
    var remind = function() {
        ...
        setTimeout(function() {
            if (!Cookies.get('kt_mobile_panel_shown')) {
                var expires = new Date(new Date().getTime() + 15 * 60 * 1000); // expire in 15 minutes from now
                Cookies.set('kt_mobile_panel_shown', 1, { expires: expires });
                offcanvas.show();
            } 
